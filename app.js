...
async function simpanTransaksi() {
  ...
  var jam    = getWaktuSekarang();
  var newRow = [tanggal, kategori, keterangan, parseInt(jumlah), tipeForm, jam];
  ...
  try {
    var params = new URLSearchParams({
      ...
      keterangan: keterangan, jumlah: parseInt(jumlah), tipe: tipeForm, jam: jam
    });
...
function renderRiwayat() {
  ...
  for (var i = 1; i < semuaData.length; i++) {
    ...
    rows.push({ row: [iso, r[1], r[2], r[3], tipe, r[5] || ''], idx: i });
  }

  rows.sort(function(a, b) {
    // Urutkan berdasarkan tanggal + jam (terbaru di atas)
    var dtA = new Date(a.row[0] + 'T' + (a.row[5] ? a.row[5] : '00:00') + ':00');
    var dtB = new Date(b.row[0] + 'T' + (b.row[5] ? b.row[5] : '00:00') + ':00');
    return dtB - dtA;
  });
    ...
    html +=
      '<div class="riwayat-item ' + kelasItem + '">' +
        '<div class="riwayat-left">' +
          ...
          '<div class="riwayat-tanggal">📅 ' + formatTanggalDisplay(r[0]) + (r[5] ? ' &nbsp;·&nbsp; 🕐 ' + r[5] : '') + '</div>' +
        '</div>' +
